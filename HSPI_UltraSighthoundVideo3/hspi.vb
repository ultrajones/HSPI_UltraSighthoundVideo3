...
Public Class HSPI
  ...
  '
  Public SighthoundVideoThread As Thread
...
#Region "HSPI - Base Plugin API"
      ...
      Try
        ...
        Select Case dv_type
          Case "Sighthound State"
            ...
            Select Case dev_value
              Case -1
                ...
                WriteMessage("Sighthound monitoring is enabled.", MessageType.Informational)
              ...
              Case -2
                ...
                WriteMessage("Sighthound monitoring is disabled.", MessageType.Informational)
          ...
          Case "Sighthound Camera"
            ...
            Select Case dev_value
              Case -1
                Dim regexPattern As String = "Sighthound(?<id>(\d+))-Camera"
                If Regex.IsMatch(dv_addr, regexPattern) = True Then
                  '
                  ' Enable
                  '
                  Dim id As Integer = Integer.Parse(Regex.Match(dv_addr, regexPattern).Groups("id").ToString())
                  SighthoundVideoAPI.EnableNetworkCamera(id, True)
                  SighthoundVideoAPI.RefreshNetCamList()
                End If
              ...
              Case -2
                ...
                If Regex.IsMatch(dv_addr, regexPattern) = True Then
                  ...
                  '
                  Dim id As Integer = Integer.Parse(Regex.Match(dv_addr, regexPattern).Groups("id").ToString())
                  SighthoundVideoAPI.EnableNetworkCamera(id, False)
                  SighthoundVideoAPI.RefreshNetCamList()
                End If
          ...
          Case "Sighthound Camera Rule"
            ...
            Select Case dev_value
              Case -1
                Dim regexPattern As String = "Sighthound\d\d\d-Rule(?<rule_id>(\d+))"
                If Regex.IsMatch(dv_addr, regexPattern) = True Then
                  '
                  ' Enable Rule
                  '
                  Dim rule_id As Integer = Integer.Parse(Regex.Match(dv_addr, regexPattern).Groups("rule_id").ToString())
                  SighthoundVideoAPI.EnableSighthoundRuleById(rule_id, True)
                End If
              ...
              Case -2
                ...
                If Regex.IsMatch(dv_addr, regexPattern) = True Then
                  ...
                  '
                  Dim rule_id As Integer = Integer.Parse(Regex.Match(dv_addr, regexPattern).Groups("rule_id").ToString())
                  SighthoundVideoAPI.EnableSighthoundRuleById(rule_id, False)
                End If
          ...
          Case "Sighthound Arming"
            ...
            Select Case dev_value
              Case -1
                SighthoundVideoAPI.EnableNetworkCameras(True)

              Case -2
                SighthoundVideoAPI.EnableNetworkCameras(False)
  ...
  Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
    ...
    Try
      ...
      '
      gSighthoundVersion = GetSetting("Sighthound", "Version", gSighthoundVersion)
      gSighthoundURL = GetSetting("Sighthound", "URL", gSighthoundURL)
      gSighthoundPort = GetSetting("Sighthound", "Port", gSighthoundPort)
      gSighthoundUsername = GetSetting("Sighthound", "Username", gSighthoundUsername)
      gSighthoundPassword = GetSetting("Sighthound", "Password", gSighthoundPassword)
      gSnapshotMaxWidth = GetSetting("Options", "SnapshotsMaxWidth", gSnapshotMaxWidth)

      '
      ' Initialize our devices
      '
      hspi_devices.FixDeviceTypes()
      hspi_devices.UpdateRootDevices()
      hspi_devices.InitPluginDevices()

      HSAppPath = hs.GetAppPath
      gSnapshotDirectory = FixPath(String.Format("{0}\html\images\hspi_ultrasighthoundvideo3\snapshots", HSAppPath))
      ...
      If bDBInitialized = True Then
        ...
        CheckDatabaseTable("tblNetCamRules")
        ...
        '
        SighthoundVideoAPI = New hspi_sighthound_api
        ...
        '
        SighthoundVideoThread = New Thread(New ThreadStart(AddressOf SighthoundVideoAPI.PollSighthoundVideoThread))
        SighthoundVideoThread.Name = "SighthoundVideoThread"
        SighthoundVideoThread.Start()

        WriteMessage(String.Format("{0} Thread Started", SighthoundVideoThread.Name), MessageType.Debug)
  ...
  Public Sub ShutdownIO() Implements HomeSeerAPI.IPlugInAPI.ShutdownIO
    ...
    Try
      ...
      'If SighthoundVideoThread.IsAlive = True Then
      '  SighthoundVideoThread.Abort()
      'End If
...
#Region "HSPI - Devices"
  ...
  Public Function ConfigDevice(ref As Integer, user As String, userRights As Integer, newDevice As Boolean) As String Implements HomeSeerAPI.IPlugInAPI.ConfigDevice
    ...
    Try
      ...
      Dim dv_addr = dv.Address(hs)
      Dim NetCamDevices As List(Of NetCamDevice) = SighthoundVideoAPI.GetNetCamDevices()
      Dim NetCamDevice As NetCamDevice = NetCamDevices.Find(Function(s) s.dv_addr = dv_addr)

      If NetCamDevice Is Nothing Then Return "Sighthound Video Camera not found."

      Dim SightHoundRules As List(Of SighthoundRule) = NetCamDevice.Rules

      stb.Append(clsPageBuilder.DivStart("div_config_device", ""))
      stb.AppendLine("<table cellspacing='0' width='100%'>")
      stb.AppendLine("<tr>")
      stb.AppendLine("<th class='tableheader' colspan='3'>Sighthound Video Camera Rules</th>")
      stb.AppendLine("</tr>")

      For Each SighthoundRule In SightHoundRules

        Dim FrmId As String = String.Format("{0}{1}", "frmConfigDevice", SighthoundRule.Id)
        Dim RbId As String = String.Format("{0}{1}", "SighthoundRule-", SighthoundRule.Id)
        Dim jqRadioButton As New clsJQuery.jqRadioButton(RbId, IFACE_NAME, True)

        jqRadioButton.values.Add("On", "1")
        jqRadioButton.values.Add("Off", "0")
        jqRadioButton.checked = IIf(SighthoundRule.Enabled = True, "1", "0")

        stb.AppendLine(" <tr>")
        stb.AppendFormat("  <td class='tablecell'>{0}</td>", SighthoundRule.Name)
        stb.AppendFormat("  <td class='tablecell'>{0}</td>", SighthoundRule.Status)
        stb.AppendFormat("  <td class='tablecell'>{0}{1}{2}</td>", clsPageBuilder.FormStart(FrmId, FrmId, "POST"), jqRadioButton.Build, clsPageBuilder.FormEnd())
        stb.AppendLine(" </tr>")
